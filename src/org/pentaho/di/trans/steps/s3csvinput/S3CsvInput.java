...
public class S3CsvInput extends BaseStep implements StepInterface {
  ...
  public S3CsvInput( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
                     Trans trans ) {
  ...
  private void prepareToRunInParallel() throws KettleException {
    try {
      ...
      logBasic( Messages.getString( "S3CsvInput.Log.ParallelFileNrAndPositionFeedback", data.filenames[ data.filenr ],
      ...
      if ( firstRow ) {
        ...
        if ( index < 0 ) {
          throw new KettleException(
            Messages.getString( "S3CsvInput.Exception.FilenameFieldNotFound", filenameField ) );
        }
      }
  ...
  private boolean openNextFile() throws KettleException {
    try {
      ...
      if ( data.parallel ) {
        ...
        data.s3Object = data.s3Service
          .getObject( data.s3bucket, data.filenames[ data.filenr ], null, null, null, null, data.bytesToSkipInFirstFile,
            data.bytesToSkipInFirstFile + data.blockToRead + data.maxLineSize * 2 );
      ...
      if ( ( meta.isHeaderPresent() && !data.parallel ) || // Standard flat file : skip header
        ( data.parallel && data.filenr == data.startFilenr && data.bytesToSkipInFirstFile <= 0 )
        // parallel processing : first file : nothing to skip
        || ( data.parallel && data.filenr > data.startFilenr
        && data.bytesToSkipInFirstFile <= 0 )   // parallel processing : start of next file, nothing to skip
      ) {
